...
import { ArrowUpRight, CheckCircle2, FileText, UsersRound, Wrench } from 'lucide-react'
...
function EvidenceRow({ contribution }: { contribution: Contribution }) {
  return (
    <Link href={`/employee/contributions#${contribution.id}`} className="group flex items-start justify-between gap-4 border-b border-white/[0.07] py-4 last:border-0">
      <div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium text-[#d7cfc4]">{contribution.title}</p><StatusBadge status={contribution.status} /></div><p className="mt-1.5 text-xs leading-5 text-[#817a72]">{contribution.description}</p></div>
...
export default function ReviewBriefPage() {
  ...
  return (
    <div className="page-shell space-y-8">
      <PageHeader eyebrow="January–September 2026" title="Review brief" description="A living summary built automatically from Maya’s source-linked meeting evidence." action={<div className="flex items-center gap-2 text-xs text-[#827b72]"><FileText size={14} className="text-[#b8a1f4]" />Updated from current record</div>} />
      ...
      <section className="topography rounded-[28px] border border-[#8d72d8]/20 bg-[#8d72d8]/[0.055] p-7 sm:p-10">
        ...
        <p className="mt-4 max-w-5xl font-serif text-2xl leading-9 text-[#ede5da] sm:text-3xl sm:leading-[1.35]">Maya improved the reliability of sample-handling, life-support, power, rover, and communications firmware for Ares Frontier’s Mars systems. She paired deep debugging with repeatable testing, helped teammates validate fixes, and increasingly coordinated integrated work across engineering groups.</p>
      </section>
      ...
      <section className="panel p-6 sm:p-8"><p className="eyebrow">Work by project</p><div className="mt-5 grid gap-4 md:grid-cols-2">{PROJECTS.map((project) => {
